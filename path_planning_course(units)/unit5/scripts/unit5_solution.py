...
def new_goal_callback(data):
    ...
    # Ensure unbounded_attractive_field is not None and is iterable
    if unbounded_attractive_field is None:
        rospy.logerr("populate_attractive_field returned None. Aborting goal callback.")
        return

    if not isinstance(unbounded_attractive_field, list):
        rospy.logerr("populate_attractive_field returned a non-list object. Aborting goal callback.")
        return
